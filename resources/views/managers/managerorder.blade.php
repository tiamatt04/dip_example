                            ...
                            <div id="collapse{{$order->id}}" class="accordion-collapse collapse show" data-bs-parent="#accordionExample">
                                <div class="accordion-body">
                                    ...
                                    @if($order->status_id == 1)
                                        <form action="{{ route('manager.order.status', ['orderId'=> $order->id, 'action'=> 'cancel']) }}" method="post">
                                            @csrf
                                        ...
                                        <form action="{{ route('manager.order.status', ['orderId'=> $order->id, 'action'=> 'confirm']) }}" method="post">
                                            @csrf
